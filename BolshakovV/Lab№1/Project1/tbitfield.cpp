#include "tbitfield.h"
#include <limits>
#include <sstream> //дл€ тестов ввода и вывода
...
int TBitField::GetMemIndex(const int n) const {  //индекс элемента pMem, в котором находитс€ бит n
	if (n < 0 || n >= BitLen) {
...
void TBitField::SetBit(const int n) //установить бит
{
	...
	pMem[GetMemIndex(n)] |= GetMemMask(n); //1 бит мен€етс€ на единицу
...
TBitField TBitField::operator~(void) {
	...
	if (bits_in_last_elem > 0) {
		TELEM mask = ((TELEM)1 << bits_in_last_elem) - 1; // маска дл€ используемых битов 
